...
class PresentationScreenBlockEntity(pos: BlockPos, state: BlockState) :
    ...
    fun applyClientSync(newFileName: String?, newIndex: Int) {
        ...
        PresentationMod.LOGGER.info("Presentation sync on client: file={}, slide={}, pos={}", fileName, currentSlideIndex, worldPosition)
        if (fileChanged) {
    ...
    override fun getRenderBoundingBox(): AABB {
        ...
        val minX = minOf(0, bounds.right.stepX * (bounds.width - 1), bounds.facing.stepX)
        val maxX = maxOf(0, bounds.right.stepX * (bounds.width - 1), bounds.facing.stepX) + 1
        val minZ = minOf(0, bounds.right.stepZ * (bounds.width - 1), bounds.facing.stepZ)
        val maxZ = maxOf(0, bounds.right.stepZ * (bounds.width - 1), bounds.facing.stepZ) + 1

        return AABB(
            bounds.controller.x + minX - 1.0,
            bounds.controller.y - 1.0,
            bounds.controller.z + minZ - 1.0,
            bounds.controller.x + maxX + 1.0,
            bounds.controller.y + bounds.height + 1.0,
